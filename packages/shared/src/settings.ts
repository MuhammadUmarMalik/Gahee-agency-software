...
export const businessSettingSchema = z.object({
  ...
  logoDataUrl: z
    .string()
    .max(1_500_000, "Encoded logo is too large.")
    .nullable()
    .refine((value) => value === null || /^data:image\/(png|jpeg|webp);base64,/i.test(value), "Logo must be a PNG, JPEG, or WebP image."),
  thankYou: plainText(160),
  thankYouUrdu: plainText(160),
  sellerNTNCNIC: z
    .string()
    .trim()
    .regex(/^$|^(?:\d{7}|\d{13})$/, "Enter a 7 digit NTN or a 13 digit CNIC.")
    .default(""),
  sellerProvince: plainText(80).default(""),
  fbrEnabled: z.boolean().default(true),
  fbrEnvironment: z.enum(["SANDBOX", "PRODUCTION"]).default("SANDBOX"),
  fbrScenarioId: z
    .string()
    .trim()
    .regex(/^SN\d{3}$/, "Scenario ID must look like SN001.")
    .default("SN000"),
});
...
export const DEFAULT_BUSINESS_SETTING: BusinessSetting = {
  name: "Oil & Ghee Agency",
  nameUrdu: "آئل اینڈ گھی ایجنسی",
  address: "Main Market, Pakistan",
  addressUrdu: "مین مارکیٹ، پاکستان",
  phone: "",
  currency: "PKR",
  timezone: "Asia/Karachi",
  logoDataUrl: null,
  thankYou: "Thank you for your business",
  thankYouUrdu: "آپ کی خریداری کا شکریہ",
  sellerNTNCNIC: "",
  sellerProvince: "",
  fbrEnabled: true,
...
export const DEFAULT_PRINTING_SETTING: PrintingSetting = {
  defaultPaper: "A4",
  autoPrint: true,
};
