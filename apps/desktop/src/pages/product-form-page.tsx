...
const defaults: CreateProductInput = { name: "", brandId: null, categoryId: null, productType: "OIL", sizeValue: null, sizeUnit: null, baseUnitId: "", packUnitId: "", unitsPerPack: 12, purchaseUnit: "PACK", saleUnit: "BASE", sku: "", barcode: "", purchasePrice: "0.00", retailPrice: "0.00", wholesalePrice: "0.00", minimumPrice: "0.00", taxRatePercent: "0.00", fbrHsCode: "", fbrUom: "Numbers, pieces, units", fbrSaleType: "Goods at standard rate (default)", fbrFixedNotifiedValue: "0.00", fbrSroScheduleNo: "", fbrSroItemSerialNo: "", reorderLevelBaseQty: 0, openingStockPackQty: 0, openingStockBaseQty: 0, rackLocation: "", notes: "", isActive: true };
...
export function ProductFormPage() {
  ...
  const form = useForm<z.input<typeof createProductInputSchema>, unknown, CreateProductInput>({ resolver: zodResolver(createProductInputSchema), defaultValues: defaults });
  ...
  return <section className="p-8"><Button variant="ghost" className="mb-5 -ml-3" onClick={() => navigate("/products")}><ArrowLeft className="mr-2" size={18}/>Back to products</Button><div className="mb-7"><p className="mb-2 text-sm font-semibold text-primary">PRODUCT MANAGEMENT</p><h1 className="m-0 text-3xl font-bold">{editing ? "Edit product" : "Add product"}</h1><p className="mt-2 text-sm text-muted-foreground">Product details, pricing, packing, and opening stock are configured on this screen.</p></div>
    ...
    <form onSubmit={handleSubmit(submit)} noValidate className="space-y-5">
      ...
      <Section title="FBR digital invoicing"><div className="mb-5 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900"><strong className="block">These values must match the reference values assigned in the FBR sandbox / production portal.</strong><span className="mt-1 block">Missing HS code, UOM, or sale type will keep invoices in review and they will not count as successful FBR scenarios.</span></div><div className="grid grid-cols-4 gap-5"><Field label="HS code" error={errors.fbrHsCode?.message}><Input placeholder="1511.9090" {...register("fbrHsCode")}/></Field><Field label="FBR UOM" error={errors.fbrUom?.message}><Input placeholder="Numbers, pieces, units" {...register("fbrUom")}/></Field><Field label="Sale type" error={errors.fbrSaleType?.message} className="col-span-2"><Input placeholder="Goods at standard rate (default)" {...register("fbrSaleType")}/></Field><Field label="Fixed / notified value" error={errors.fbrFixedNotifiedValue?.message}><Input inputMode="decimal" {...register("fbrFixedNotifiedValue")}/></Field><Field label="SRO schedule no." error={errors.fbrSroScheduleNo?.message}><Input {...register("fbrSroScheduleNo")}/></Field><Field label="SRO item serial no." error={errors.fbrSroItemSerialNo?.message}><Input {...register("fbrSroItemSerialNo")}/></Field></div></Section>
...
function productToForm(product: ProductDto): CreateProductInput { return { name: product.name, brandId: product.brand?.id ?? null, categoryId: product.category?.id ?? null, productType: product.productType, sizeValue: product.sizeValue, sizeUnit: product.sizeUnit, baseUnitId: product.baseUnit.id, packUnitId: product.packUnit.id, unitsPerPack: product.unitsPerPack, purchaseUnit: product.purchaseUnit, saleUnit: product.saleUnit, sku: product.sku, barcode: product.barcode, purchasePrice: product.purchasePrice, retailPrice: product.retailPrice, wholesalePrice: product.wholesalePrice, minimumPrice: product.minimumPrice, taxRatePercent: product.taxRatePercent, fbrHsCode: product.fbrHsCode, fbrUom: product.fbrUom, fbrSaleType: product.fbrSaleType, fbrFixedNotifiedValue: product.fbrFixedNotifiedValue, fbrSroScheduleNo: product.fbrSroScheduleNo, fbrSroItemSerialNo: product.fbrSroItemSerialNo, reorderLevelBaseQty: product.reorderLevelBaseQty, openingStockPackQty: 0, openingStockBaseQty: 0, rackLocation: product.rackLocation, notes: product.notes, isActive: product.isActive }; }
