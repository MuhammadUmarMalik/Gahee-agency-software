...
type SupplierRecord = { openingBalanceMinor: number; [key: string]: unknown };

export class SupplierService {
  constructor(private readonly db: AppDbClient) {}

  async list(query: SupplierListQuery) {
    const where = { deletedAt: null, ...(query.active === "ACTIVE" ? { isActive: true } : query.active === "INACTIVE" ? { isActive: false } : {}), ...(query.search ? { OR: [{ name: { contains: query.search } }, { businessName: { contains: query.search } }, { phone: { contains: query.search } }, { whatsapp: { contains: query.search } }, { code: { contains: query.search } }, { taxIdentifier: { contains: query.search } }] } : {}) };
    ...
    const purchases = await this.db.purchase.groupBy({ by: ["supplierId"], where: { supplierId: { in: suppliers.map((supplier) => supplier.id) }, status: "POSTED" }, _max: { purchasedAt: true } });
  ...
  async get(id: string) {
    ...
    const age = supplierAging(entries), last = await this.db.purchase.findFirst({ where: { supplierId: id, status: "POSTED" }, orderBy: { purchasedAt: "desc" }, select: { purchasedAt: true } });
  ...
  async create(input: CreateSupplierInput, userId: string) {
    ...
    } catch (error) { if (isUniqueConstraintError(error)) throw new HttpError(409, "DUPLICATE_SUPPLIER", "Supplier code already exists. Try again."); throw error; }
  }
  ...
  async update(id: string, input: UpdateSupplierInput, userId: string) {
    ...
    await this.db.$transaction(async (tx) => { await tx.supplier.update({ where: { id }, data: { name: input.name, businessName: input.businessName || null, phone: input.phone || null, whatsapp: input.whatsapp || null, address: input.address || null, taxIdentifier: input.taxIdentifier || null, isActive: input.isActive } }); await tx.auditLog.create({ data: { userId, action: "UPDATE", entityType: "Supplier", entityId: id, beforeJson: JSON.stringify(before), afterJson: JSON.stringify(input) } }); });
    return this.get(id);
  }
  ...
  async remove(id: string, userId: string) {
    ...
    await this.db.$transaction(async (tx) => { await tx.supplier.update({ where: { id }, data: { isActive: false, deletedAt: new Date() } }); await tx.auditLog.create({ data: { userId, action: "SOFT_DELETE", entityType: "Supplier", entityId: id, beforeJson: JSON.stringify(supplier) } }); });
  }
  ...
  private dto(supplier: SupplierRecord, payableMinor: number, overdueMinor: number, lastPurchaseAt: Date | null) { return { ...supplier, openingBalance: minorToMoney(supplier.openingBalanceMinor), payable: minorToMoney(payableMinor), overdue: minorToMoney(overdueMinor), lastPurchaseAt: lastPurchaseAt?.toISOString() ?? null }; }
}
