...
export class AccountingController {
  ...
  updateAccount = async (req: Request, res: Response) => res.json(await this.service.updateAccount(String(req.params.id), accountUpdateSchema.parse(req.body), req.auth!.id));
  ...
  changePeriod = async (req: Request, res: Response) => { const input = closePeriodSchema.parse(req.body); res.json(await this.service.changePeriod(String(req.params.id), input.action, req.auth!.id)); };
  journals = async (req: Request, res: Response) => res.json(await this.service.journals(journalFilterSchema.parse(req.query)));
  journal = async (req: Request, res: Response) => res.json(await this.service.journal(String(req.params.id)));
  ...
  reverseJournal = async (req: Request, res: Response) => { const input = reversalInputSchema.parse(req.body); res.status(201).json(await this.service.reverseJournal(String(req.params.id), input.reason, req.auth!.id)); };
  reversePayment = async (req: Request, res: Response) => { const input = reversalInputSchema.parse(req.body); res.status(201).json(await this.service.reversePayment(String(req.params.id), input.reason, req.auth!.id)); };
  ...
  generalLedger = async (req: Request, res: Response) => res.json(await this.service.generalLedger(String(req.params.accountId), financialReportFilterSchema.parse(req.query)));
  cashInHand = async (req: Request, res: Response) => res.json(await this.service.cashInHand(financialReportFilterSchema.parse(req.query)));
